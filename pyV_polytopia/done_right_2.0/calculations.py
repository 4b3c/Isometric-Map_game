...
class calculations:
	...
	def get_window_coords(coords):
		x = coords[0]
		y = coords[1]
		t_x = (var.mid_x) + (x * var.tile_size) - (y * var.tile_size)
		t_y = ((var.mid_y) - (y * var.half_tile) - (x * var.tile_size) / 2) + var.half_tile
	...
	def get_grid_coords(coords):
		...
		g_x = round(math.sqrt(((inter_x[0] - 0)**2) + ((inter_x[1] - 0)**2)) / var.tile_hyp) * negCx
		inter_y = calculations.intersection(real_x, real_y, -1)
		g_y = round(math.sqrt(((inter_y[0] - 0)**2) + ((inter_y[1] - 0)**2)) / var.tile_hyp) * negCy
