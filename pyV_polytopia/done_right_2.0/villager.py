...
class villager:
	def __init__(self):
		self.coords_g = [0, 0]
		self.offset = [0, 0]
		self.speed = 0.25 * var.game_speed
	...
	def move_villager(self, targ_coords_g):
		...
		# This let's it go in straight lines to the place it wants to go
			...
			if round(villager.get_exact_coords(self)[1]) == round(calculations.get_window_coords((self.coords_g[0], self.coords_g[1] + direction_y))[1] - var.half_tile):
